...
export default function ItemDetails() {
  ...
  const item = useDbStore((s) => s.items.find((i) => i.id === id));
  const addClaim = useDbStore((s) => s.addClaim);
  const deleteItem = useDbStore((s) => s.deleteItem);
  ...
  const handleDelete = () => {
    deleteItem(item.id);
    toast({ title: "Item deleted", description: "Your item has been removed.", variant: "destructive" });
    navigate("/browse");
  };
            ...
            <div className="relative aspect-square rounded-xl overflow-hidden bg-muted">
              ...
              <div className="absolute top-4 left-4 flex gap-2">
                <Badge variant={item.type === "lost" ? "destructive" : "default"}
                  className={`text-sm ${item.type === "found" ? "bg-success hover:bg-success/90" : ""}`}>
              ...
              <Card>
                <CardContent className="p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-foreground">{format(new Date(item.date), "MMM d, yyyy")}</p>
                  </div>
                </CardContent>
              </Card>
            </div>
            ...
            {isOwner ? (
              ...
              <>
                <Button className="w-full gap-2" size="lg" onClick={handleClaimClick}>
                  <CheckCircle className="h-5 w-5" /> Claim This Item
                </Button>
                <Dialog open={claimDialogOpen} onOpenChange={setClaimDialogOpen}>
                  <DialogContent>
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Tell us why you believe this item belongs to you. The finder will review your claim.
                      </DialogDescription>
                    </DialogHeader>
                      ...
                      {({ errors, touched, isSubmitting }) => (
                        <Form className="space-y-4">
                          <div className="space-y-2">
                            ...
                            <Field as={Textarea} id="reason" name="reason" placeholder="Describe identifying features..."
                              rows={3} className={errors.reason && touched.reason ? "border-destructive" : ""} />
                            {errors.reason && touched.reason && <p className="text-sm text-destructive">{errors.reason}</p>}
                          </div>
                          <div className="space-y-2">
                            <Label htmlFor="details">Additional details (optional)</Label>
                            <Field as={Textarea} id="details" name="details" placeholder="Any other information..."
                              rows={2} className={errors.details && touched.details ? "border-destructive" : ""} />
              ...
              <Button className="w-full gap-2" size="lg" onClick={() => {
                if (!isAuthenticated) {
                  toast({ title: "Login required", description: "Please log in to contact the owner.", variant: "destructive" });
                  navigate("/login");
                  return;
                }
                navigate(`/messages?item=${item.id}`);
              }}>
                <MessageSquare className="h-5 w-5" /> I Found This Item
              </Button>
            ) : null}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      Member since {format(new Date(item.user.memberSince), "MMMM yyyy")}
                    </p>
                  </div>
                  {!isOwner && (
