...
export default function Messages() {
  const { user } = useAuth();
  const dbConversations = useDbStore((s) => s.conversations);
  const dbMessages = useDbStore((s) => s.messages);
                ...
                filteredConversations.map((conv) => {
                  ...
                  return (
                    <button key={conv.id} onClick={() => setSelectedConversation(conv.id)}
                      className={cn("w-full p-4 flex items-start gap-3 hover:bg-muted/50 transition-colors border-b", selectedConversation === conv.id && "bg-muted")}>
                      <div className="relative">
                ...
                <div className="p-4 border-t">
                  <div className="flex items-center gap-2">
                    ...
                    <Input placeholder="Type a message..." value={messageInput} onChange={(e) => setMessageInput(e.target.value)}
                      onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSendMessage(); } }} className="flex-1" />
