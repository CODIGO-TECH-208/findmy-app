                        ...
                        <div className="space-y-4">
                            {recentItems.map((item) => (
                                <div key={item.id} className="flex items-start gap-4 pb-4 border-b border-gray-100 last:border-0 last:pb-0">
                                    ...
                                    <span className={cn("text-xs px-2 py-1 rounded-full font-medium whitespace-nowrap",
                                        item.status === "active" ? "bg-green-100 text-green-700" : item.status === "claimed" ? "bg-yellow-100 text-yellow-700" : "bg-gray-100 text-gray-700"
                                    )}>{item.status}</span>
                                </div>
                            ))}
                        </div>
                            ...
                            <div className="space-y-4">
                                {recentClaims.map((claim) => (
                                    <div key={claim.id} className="flex items-start gap-4 pb-4 border-b border-gray-100 last:border-0 last:pb-0">
                                        ...
                                        <span className={cn("text-xs px-2 py-1 rounded-full font-medium whitespace-nowrap",
                                            claim.status === "pending" ? "bg-yellow-100 text-yellow-700" : claim.status === "accepted" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"
                                        )}>{claim.status}</span>
                                    </div>
                                ))}
                            </div>
